...
After a long time without any updates, now I will take a time for see the opened issues, pull requests and bug fixies.
Please contact me for more info or help.
...
1. Follow the Redmine plugin installation steps at: http://www.redmine.org/wiki/redmine/Plugins Make sure the plugin is installed to <tt>vendor/plugins/redmine_time_tracker</tt>
...
to copy <tt>redmine_time_tracker/assets/*</tt> to <tt><redmine_root>/plugin_assets/redmine_time_tracker/</tt>
or to make a symbolic link <tt><redmine_root>/plugin_assets/redmine_time_tracker</tt> pointing to <tt>redmine_time_tracker/assets</tt>
...
2. Replace your old Redmine's <tt>vendor/plugins/redmine_time_tracker</tt> folder with the one contained in the downloaded package
3. Update the database using the migrations. <tt>rake db:migrate_plugins RAILS_ENV=production</tt>
4. Restart your Redmine

<b>Note:</b> if you have done a copy of the plugin assets into <tt><redmine_root>/plugin_assets/redmine_time_tracker</tt>
as explained in the _Install_ paragraph above, you will need to update these too.

=== Git

1. Open a shell to your Redmine's <tt>vendor/plugins/redmine_time_tracker</tt> folder
